...
type ModoAdicionarPdi = 'TECNICA' | 'COMPORTAMENTAL' | 'CERTIFICACAO';

const TITULO_MODAL: Record<ModoAdicionarPdi, string> = {
  TECNICA: 'Adicionar competência técnica',
  COMPORTAMENTAL: 'Adicionar competência comportamental',
  CERTIFICACAO: 'Adicionar certificação',
};

/**
 * Adicionar manualmente ao PDI — pedido do utilizador: "deve ser possível
 * adicionar... manualmente", dividido em 3 entradas (técnica, comportamental,
 * certificação) em vez de um seletor "Tipo" dentro do modal — o botão
 * clicado já diz o que se está a adicionar. Competência exige sempre um
 * nível-alvo (pedido do utilizador: "senão não temos um target de
 * atingimento para perceber se cumpriu ou não"); Certificação não tem
 * escala de nível.
 */
function AdicionarPdiItemModal({ modo, colaboradorId, onClose }: { modo: ModoAdicionarPdi; colaboradorId: number; onClose: () => void }) {
  const queryClient = useQueryClient();
  const [itemId, setItemId] = useState('');
  const [nivelAlvoId, setNivelAlvoId] = useState('');
  const [erro, setErro] = useState<string | null>(null);

  const ehCompetencia = modo !== 'CERTIFICACAO';
  ...
  const adicionar = useMutation({
    mutationFn: () =>
      endpoints.pdiCriar(
        colaboradorId,
        ehCompetencia ? { competenciaId: Number(itemId), nivelAlvoId: Number(nivelAlvoId) } : { certificacaoId: itemId },
      ),
    onSuccess: () => {
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!itemId || (ehCompetencia && !nivelAlvoId)) return;
    setErro(null);
    adicionar.mutate();
  }

  return (
    <Modal title={TITULO_MODAL[modo]} onClose={onClose}>
      <form onSubmit={handleSubmit}>
        <Field label={ehCompetencia ? 'Competência' : 'Certificação'}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={adicionar.isPending || !itemId || (ehCompetencia && !nivelAlvoId)}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {item.nivelAlvo && (
          <p className="mt-1 text-xs text-fiori-text-secondary">
            Nível a atingir: <span className="font-medium">{item.nivelAlvo.nome}</span>
          </p>
        )}
        {item.formacao && (
          <p className="mt-1 text-xs text-fiori-text-secondary">
    ...
    <Card
      ...
      action={
        <div className="flex flex-wrap gap-2 no-print">
          <Button variant="secondary" onClick={() => setAAdicionar('TECNICA')}>
            <span className="flex items-center gap-1.5">
              <Plus size={14} /> Adicionar competência técnica
            </span>
          </Button>
          <Button variant="secondary" onClick={() => setAAdicionar('COMPORTAMENTAL')}>
            <span className="flex items-center gap-1.5">
              <Plus size={14} /> Adicionar competência comportamental
            </span>
          </Button>
          <Button variant="secondary" onClick={() => setAAdicionar('CERTIFICACAO')}>
            <span className="flex items-center gap-1.5">
    ...
    >
      {isLoading ? (
        ...
        <p className="text-sm text-fiori-text-secondary">
          Ainda sem itens de PDI. Clica em "Gerar sugestões" para criar um plano a partir dos gaps deste colaborador, ou num dos botões
          "Adicionar" para escolher manualmente.
        </p>
      ) : (
        (() => {
